...
//float y[250]={0.0};
//int index;
if ((infile=fopen("flow.txt", "r")) == NULL)
...
// 	printf("Open file successfully!\n");
	...
	//	if (strcmp(id,string)==0){
		 ...
		 //printf("%s",id);
			//fscanf(infile, "%s ", &id);
			//printf("%s",id);
			while(fscanf(infile, "%d, %d, %lf", &i, &j, &c)!=EOF){
			capacity[i][j]=c;
			//fscanf(infile, "%d, %d, %lf", &i, &j, &c);
			//fscanf(infile, "%s", &id);
			//fscanf(infile, "%d,", &u);
...
//	}
if ((outfile=fopen("Flow.txt", "w")) == NULL)
	printf("Fail to open file!");
else
	printf("Open file successfully!\n");
